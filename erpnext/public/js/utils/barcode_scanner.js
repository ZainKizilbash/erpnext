...
erpnext.utils.setup_scan_barcode_field = (field) => {
	if (!field) {
		return;
	}
...
erpnext.utils.BarcodeScanner = class BarcodeScanner {
	constructor(opts) {
		...
		// }
		this.scan_api = opts.scan_api || "erpnext.stock.get_item_details.scan_barcode";
	}
		...
		return new Promise((resolve, reject) => {
			...
			frappe.run_serially([
				() => this.set_item(row, item_code, barcode, batch_no, serial_no).then(qty => {
					this.show_scan_message(row.idx, row.item_code, !this.is_new_row, qty);
				}),
